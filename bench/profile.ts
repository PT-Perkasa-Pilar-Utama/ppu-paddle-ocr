...
try {
  const privateImg = Bun.file(
    import.meta.dir + "/../private-tests/private-test-1.png",
  );
...
console.log(
  `Images: ${images.length} (receipt${privateImageBuffer ? " + private-test-1" : ""})\n`,
);
...
for (const engine of ["opencv", "canvas-native"] as const) {
  ...
  await service.recognize(images[0]!, { noCache: true });
